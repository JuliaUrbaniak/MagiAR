  ...
  async function handleCastSpell() {
    ...
    const interval = setInterval(() => {
      ...
      if (seconds === 0) {
        ...
        // ✨ CZAROWANIE trwa przez np. 5 sekund (czas na wykrycie lasera, zbieranie punktów)
        setTimeout(async () => {
          setShowCastingText(false);

          // TODO: Tutaj rozpoczniesz analizę klatek — zbieranie trajektorii
          const image = captureFrame();
          ...
          //setDetectedSpell('Glacius_V');

          // Ukryj napis po np. 4 sekundach
          setTimeout(() => {
            setDetectedSpell(null);
          }, 4000);

        }, 5000); // tutaj można później dać timeout oparty o wykrycie braku lasera
      }
    }, 1000);
  }
